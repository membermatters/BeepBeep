...
class SlimDNSServer:
    ...
    def _make_socket(self):
        ...
        member_info = dotted_ip_to_bytes(
            _MDNS_ADDR) + dotted_ip_to_bytes(self.local_addr)
    ...
    def advertise_hostname(self, hostname, find_vacant=True):
        ...
        A_record = pack_answer(
            hostname, _TYPE_A, _CLASS_IN, _DNS_TTL, ip_bytes)
    ...
    def process_packet(self, buf, addr):
        ...
        (pkt_id, flags, qst_count, ans_count,
         _, _) = unpack_from("!HHHHHH", buf, 0)
        o = 12
        matches = []
        reply_len = 12
        ...
        self.sock.sendto(buf[:o], (_MDNS_ADDR, _MDNS_PORT)
                         if addr[0] == _MDNS_PORT else addr)
